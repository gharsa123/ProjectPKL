...
const AddServer: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const endpoint = getEndpoint("servers", selectedCompany);
      if (endpoint) {
        const response = await axios.post(endpoint, formData);
        if (response.status === 201) {
          setTimeout(() => {
            navigate(`/servers`, { state: { showToast: true } });
          }, 300);
        } else {
  ...
  return (
      <div className="bg-white p-10 rounded-3xl w-full h-auto">
        <div className="flex justify-center mb-6">
          ...
          <h1 className="font-semibold mb-3 text-2xl">Add New Server</h1>
        </div>
        <label
          htmlFor="company"
