...
const AddApplication: React.FC = () => {
  ...
  const getEndpoint = (category: string, company: string): string | null => {
    const categoryEndpoints =
      endpointbyID[category as keyof typeof endpointbyID];
  ...
  const handleSubmit = async () => {
    if (!formData.vm_id || !selectedCompany || !formData.ip_public) {
    ...
    try {
      const endpoint = getEndpoint("aplikasi", selectedCompany);
      console.log("Generated endpoint: ", endpoint);
      if (endpoint) {
        const finalEndpoint = endpoint.replace(":id", formData.vm_id);
        const response = await axios.post(finalEndpoint, formData);

        if (response.status === 201) {
          setTimeout(() => {
            navigate(`/aplikasi`, { state: { showToast: true } });
          }, 300);
        } else {
    ...
    <div className="bg-white p-12 rounded-3xl w-full h-auto">
      <form>
        <div className="flex justify-center mb-6">
          ...
          <h1 className="font-semibold mb-3 text-2xl">Add New Application</h1>
        </div>
        <label
          htmlFor="company"
