...
const DomainBody: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const endpoint = getEndpoint("domain", company);
        if (endpoint) {
    ...
    fetchData();
  }, [company, itemsPerPage, currentPage]);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const categoryEndpoints =
        endpointbyID["domain" as keyof typeof endpointbyID];
  ...
  const filteredData = data.filter(
    (item) =>
      ...
      item.d_omain?.toLowerCase().includes(searchTerm.toLowerCase()) ||
              ...
              <table className="min-w-full mb-7 bg-[#34D5F4]">
                <thead>
                  <tr>
                    ...
                    <th className="border text-white border-white">Port</th>
                    ...
                    >
                      ...
                      <td className="border border-white text-center">
                        {editItemId === item.id ? (
                          <Input
                            type="text"
                            value={editData?.d_omain || ""}
                            onChange={(e) =>
                              setEditData({
                                ...editData!,
                                d_omain: e.target.value,
                              })
                            }
                            className="border rounded p-2"
                          />
