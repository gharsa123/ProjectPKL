...
const ApplicationBody: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const endpoint = getEndpoint("aplikasi", company);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const categoryEndpoints =
        endpointbyID["aplikasi" as keyof typeof endpointbyID];
  ...
  const filteredData = data.filter(
    (item) =>
      item.vm_id.toString().includes(searchTerm.toLowerCase()) ||
      item.ip_public.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.port_int.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.port_ext.toLowerCase().includes(searchTerm.toLowerCase()) ||
          ...
          <div className="flex flex-col">
            ...
            {filteredData.length > 0 ? (
              <table className="min-w-full mb-7 bg-[#34D5F4]">
                <thead>
                  <tr>
                    <th className="border text-white border-white">Vm Id</th>
                    <th className="border text-white border-white">
                      Ip Public
                    </th>
                    <th className="border text-white border-white">Port Int</th>
                    <th className="border text-white border-white">Port Ext</th>
                    <th className="border text-white border-white">Domain</th>
                    <th className="border text-white border-white">App Name</th>
                    ...
                    >
                      ...
                      <td className="border border-white text-center">
                        {editItemId === item.id ? (
                          <Input
                            type="text"
                            value={editData?.port_int || ""}
                            onChange={(e) =>
                              setEditData({
                                ...editData!,
                                port_int: e.target.value,
                              })
                            }
                            className="border rounded p-2"
                          />
                      ...
                      <td className="border border-white text-center">
                        {editItemId === item.id ? (
                          <Input
                            type="text"
                            value={editData?.port_ext || ""}
                            onChange={(e) =>
                              setEditData({
                                ...editData!,
                                port_ext: e.target.value,
                              })
                            }
                            className="border rounded p-2"
                          />
